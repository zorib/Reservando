...
Listado.prototype.buscarRestaurante = function(id) {
  var encontrarRestaurant = this.restaurantes.find(restaurant => restaurant.id == id); (1)
  if (encontrarRestaurant === undefined) {
    return "No se ha encontrado ningún restaurant";
    } else {
      return encontrarRestaurant;
      }
    //refactoring:
...
Listado.prototype.obtenerCiudades = function() {
    ...
    var c2 = ciudades.filter(function(elem, index, self) {
...
Listado.prototype.obtenerRubros = function() {
    var rubro = [];
    rubro = this.restaurantes.map(function(restaurant){
      return restaurant.rubro;
    })

    var r2 = rubro.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });
    return r2.sort();
}
//Obtiene todos los horarios de los restaurantes
Listado.prototype.obtenerHorarios = function() {

    var horario = [];
    horario = this.restaurantes.map(function(restaurant){
      return restaurant.horarios;
    })

    var h = [];
    horario.forEach(function(a){
